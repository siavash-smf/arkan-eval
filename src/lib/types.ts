...
export type CaseResult = {
  ...
  error?: string;
  /**
   * توکن‌های مصرفی داورها برای همین کیس.
   *
   * چرا به‌ازای هر کیس ذخیره می‌شود و نه فقط جمع کل؟ چون اجرا
   * تکه‌تکه پیش می‌رود و هر تکه در یک فراخوانی جداگانه‌ی سرور
   * اجرا می‌شود. متغیر درون‌حافظه‌ای بین تکه‌ها زنده نمی‌ماند،
   * پس مصرف را همان‌جا کنار نتیجه می‌نویسیم و آخر کار جمع می‌زنیم.
   */
  judgeUsage?: { in: number; out: number };
};
...
export type RunSummary = {
  ...
  /**
   * سلامت خودِ اندازه‌گیری — نه سلامت بات.
   *
   * چرا لازم است؟ چون یک‌بار واقعاً پیش آمد: اعتبار OpenRouter تمام شد،
   * چت‌بات برای ۲۲ کیس پاسخ خالی برگرداند، و ابزار با اطمینان کامل نوشت
   * «نمره‌ی کل: ۱۹/۱۰۰». هرکس آن گزارش را می‌دید فکر می‌کرد بات خراب
   * شده، در حالی که فقط پول تمام شده بود.
   *
   * «صفرِ دروغین» از «خطای صریح» خطرناک‌تر است، چون کسی نمی‌فهمد
   * اندازه‌گیری خراب شده. پس وقتی سهم کیس‌های بی‌پاسخ از آستانه بگذرد،
   * اجرا نامعتبر علامت می‌خورد و هیچ‌جا نباید به‌عنوان نمره ارائه شود.
   *
   * اختیاری است چون اجراهای ذخیره‌شده پیش از این گارد آن را ندارند؛
   * نبودش یعنی «سنجیده نشده»، نه «خراب» (نگاه کنید به isTrustworthy).
   */
  integrity?: {
    /** کیس‌هایی که پاسخ خالی گرفتند (بدون خطای صریح). */
    emptyCount: number;
    /** کیس‌هایی که با خطا شکست خوردند. */
    errorCount: number;
    /** نسبت کیس‌های بی‌نتیجه به کل — ۰ تا ۱. */
...
export type EvalRun = {
   ...
   *
   * `caseIds` نقشه‌ی اجراست — فهرست کیس‌هایی که قرار بود اجرا شوند،
   * بعد از اعمال فیلتر دسته و سقف تعداد. با داشتن آن، هر فراخوانی
   * می‌تواند بفهمد چه چیزی مانده و اجرا را از همان‌جا ادامه بدهد،
   * بدون اینکه لازم باشد فیلترها دوباره جایی نگه داشته شوند.
   */
  progress: { done: number; total: number; caseIds?: string[] };
  results: CaseResult[];
