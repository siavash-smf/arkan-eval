...
const FINDINGS: {
  ...
  {
    title: "پاسخ‌ها وسط جمله قطع می‌شدند — رفع شد ✅",
    severity: "fail",
    signal:
    ...
    diagnosis:
      "با curl ساده هم بازتولید شد، پس اشکال از ابزار ارزیابی نبود. ریشه: max_tokens در جدول model_config روی ۸۰۰ بود و توکن‌های reasoning از همان بودجه خرج می‌شوند — چون مقدارشان متغیر است، باقی‌مانده‌ی قابل‌نمایش هم متغیر می‌شود.",
    action:
      "max_tokens در /admin/models به ۲۰۰۰ رسانده شد و ارزیابی دوباره اجرا شد. نمره‌ی کل ۹۴ → ۹۷، کوتاه‌ترین پاسخ ۸۱ → ۱۲۴ کاراکتر (جمله‌ی کامل)، و هر چهار کیس ⚠️ به ✅ تبدیل شدند.",
    lesson:
      "این باگ ماه‌ها زنده بود، هیچ خطایی در لاگ نمی‌انداخت، و در تست دستی هم دیده نمی‌شد. ارزیابی خودکار دقیقاً برای همین وجود دارد — و اجرای دوم دقیقاً برای این است که ثابت کند درمان جواب داده، نه اینکه امیدوار باشیم.",
  },
  {
    ...
    action:
      "این باگ فنی نیست، یک تصمیم محصولی است که هرگز گرفته نشده بود: آیا بات باید قیمت بدهد یا نه؟ یکی از این دو باید عوض شود و آدم‌ها باید تصمیم بگیرند. هنوز باز است.",
    lesson:
      "در اجرای دوم همین کیس ۹۵ گرفت و ✅ شد — بدون اینکه رفتار بات ذره‌ای عوض شود؛ فقط داور این‌بار نوشت «ارائه بازه‌ی قیمتی برای شفافیت مفید است». یعنی نمره‌ی بهتر همیشه به‌معنای محصول بهتر نیست. جایی که گلدن‌ست با پایگاه دانش تناقض دارد، داور تکیه‌گاه ندارد و بی‌ثبات می‌شود — حتی با دمای صفر. دقیقاً برای همین صفحه‌ی «داورِ داور» وجود دارد.",
  },
  {
